...
def picture_edge_to_01(root_path, create_picture_size, scale):
    ...
    overlay_image_0 = Image.open('0.png')
    overlay_image_1 = Image.open('1.png')
    overlay_image_0_white = Image.open('0_white.png')
    overlay_image_1_white = Image.open('1_white.png')
                ...
                for short in range(0, size0, 10):
                    ...
                    if image[int(long / 15 * scale1):int(long / 15 * scale1 + scale1),
                       int(short / 10 * scale0):int(short / 10 * scale0 + scale0)].sum() != 0:
                        base_image.paste(overlay_image_1_white, (short, long))
                    else:
                        base_image.paste(overlay_image_0, (short, long))
            print(long / 15 * scale1, long / 15 * scale1 + scale1, short / 10 * scale0, short / 10 * scale0 + scale0)
            output_image_path = os.path.join(output_image_path0, f'{i}.jpg')
...
root_path = "E:\A_WORK\LeetCode\Python\zhang_xue_liang/"
vedio_name = "zhang_xue_liang.mp4"
# 181 1135
# 155  1175
cut1 = 420
cut2 = 860
